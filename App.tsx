...
import { PlayerAsset, TreeAsset, CarAsset, LogAsset, CoinAsset, FlowerAsset, RockAsset, LilypadAsset, SplatAsset, SplashAsset, CoinCollectEffect, PowerUpAsset, TrainAsset, WarningLightAsset } from './components/Assets';
...
function App() {
  ...
  const gameRef = useRef({
    ...
    lastTime: 0,
    cameraY: 0,
    autoScrollY: 0, // For Survival Mode
    gameOver: false,
    rowIdCounter: 0,
    currentScore: 0,
  ...
  useEffect(() => {
      gameModeRef.current = activeGameMode;
  }, [activeGameMode]);
  ...
  const generateRow = (yIndex: number, difficulty: number): RowData => {
    ...
    return { id, type, obstacles, decorations, items, laneSpeed: speed, direction };
  };
  ...
  const handleInput = useCallback((key: string) => {
    ...
    if ((key === 'e' || key === 'E') && inventoryRef.current) {
        ...
        if (type === 'TIME_SLOW') playTimeSlow();
        else playCashout(); 
    ...
    // Bound Check (Y) - Cannot go below camera/void
    ...
    // Slow down player animation if TIME_SLOW is active
    const jumpDuration = activeEffectRef.current?.type === 'TIME_SLOW' ? 200 : 120;
    ...
    setTimeout(() => {
        ...
        if (nextY > gameRef.current.currentScore) {
            ...
            if (gameModeRef.current === 'GAMBLE') {
                setGambleMultiplier(prev => parseFloat((prev + 0.1).toFixed(1)));
            }
        }
        
        // Camera Movement (Classic/Gamble/Challenge)
        // Survival moves camera automatically in update loop
        ...
        // Keep buffer larger for survival to prevent running out
        if (highestRowId < (gameModeRef.current === 'SURVIVAL' ? gameRef.current.autoScrollY : gameRef.current.cameraY) + 20) {
            ...
            if (gameRef.current.rows.length > 35) {
             ...
             if (itemIndex !== -1) {
                 ...
                 if (item.type === 'COIN') {
                     ...
                     if (gameModeRef.current === 'CHALLENGE') {
                         setChallengeCoins(c => c + multiplier);
                     }
  ...
  const handleGameOver = (reason: 'CAR' | 'WATER' | 'BOUNDS' | 'VOID' | 'TRAIN' = 'CAR') => {
    ...
    if (activeEffectRef.current?.type === 'SHIELD' && reason !== 'VOID') {
        ...
        playCashout(); 
        if (reason === 'WATER') {
             gameRef.current.player.gridY -= 1;
        }
        return; 
    }
  ...
  const handleChallengeWin = () => {
      ...
      if (activeChallenge) {
          setCoins(c => c + activeChallenge.reward);
      }
  ...
  const update = useCallback((time: number) => {
    ...
    if (gameModeRef.current === 'CHALLENGE' && activeChallenge) {
        setChallengeTimer(prev => {
            ...
            if (newVal <= 0) {
                handleGameOver('BOUNDS'); // Time out
                return 0;
            }
            return newVal;
        });
        ...
        // Check win condition
        if (activeChallenge.type === 'SCORE' && gameRef.current.currentScore >= activeChallenge.target) {
            handleChallengeWin();
        } else if (activeChallenge.type === 'COINS' && challengeCoins >= activeChallenge.target) {
            handleChallengeWin();
        }
    }

    // SURVIVAL LOGIC (Auto Scroll)
    if (gameModeRef.current === 'SURVIVAL') {
        // Speed increases by score
        ...
        // Check if swallowed by void
        if (gameRef.current.player.gridY < gameRef.current.autoScrollY - 0.5) {
            handleGameOver('VOID');
        }
    }
    ...
    if (activeEffectRef.current) {
        if (activeEffectRef.current.type !== 'SHIELD') {
            ...
            if (activeEffectRef.current.timeLeft <= 0) {
                activeEffectRef.current = null;
                setActiveEffect(null);
            }
        }
        
        if (activeEffectRef.current?.type === 'TIME_SLOW') {
            timeScale *= 0.5; 
        }
    }
    ...
    if (activeEffectRef.current?.type === 'MAGNET') {
        ...
        const range = 3;
        rows.forEach(row => {
            if (Math.abs(row.id - playerY) <= range && row.items) {
       ...
       if (row.type === RowType.RAIL && row.trainState) {
           ...
           if (row.trainState.state === 'IDLE') {
               if (row.trainState.timer <= 0) {
                   row.trainState.state = 'WARNING';
                   row.trainState.timer = 2000; // Warning duration
                   playTrainWarning();
               }
           } else if (row.trainState.state === 'WARNING') {
               if (row.trainState.timer <= 0) {
                   row.trainState.state = 'PASSING';
                   row.trainState.timer = 1500; // Train pass duration
                   playTrainPass();
                   // Spawn Train
                   row.obstacles = [{
                       id: Math.random(),
                       x: row.direction === 1 ? -5 : GAME_CONFIG.COLUMNS + 5,
                       speed: 0.8, // Very fast
                       type: 'TRAIN',
                       width: 8, // Very long
               ...
               if (row.trainState.timer <= 0) {
                   row.trainState.state = 'IDLE';
                   row.trainState.timer = randInt(3000, 8000);
                   row.obstacles = []; // Despawn train
               }
           }
       }
        ...
        if (currentRow) {
            ...
            if (currentRow.type === RowType.RAIL) {
                currentRow.obstacles.forEach(train => {
                    const trainCenter = train.x + (train.width/2);
                    const playerCenter = player.gridX + 0.5;
                    const dist = Math.abs(trainCenter - playerCenter);
                    
                    // Train hitbox
                    if (dist < (train.width * 0.48)) {
                        handleGameOver('TRAIN');
                    }
                })
            }
  ...
  const handleCashout = () => {
      if (activeGameMode !== 'GAMBLE') return;
      playCashout(); 
    ...
    <div className="w-full h-screen bg-[#202028] relative overflow-hidden select-none font-pixel text-slate-100">
            ...
            {activeGameMode === 'GAMBLE' ? (
                ...
                <div className="bg-black/80 border-2 border-purple-500 px-4 py-2 shadow-lg">
                    ...
                    <div className="text-2xl text-white flex gap-4">
                        <span>{activeChallenge.type === 'SCORE' ? score : challengeCoins} / {activeChallenge.target}</span>
                        <span className={challengeTimer < 10 ? 'text-red-500 animate-pulse' : 'text-white'}>
            ...
            <div className="mt-2 pointer-events-auto">
                 <div className="bg-black/60 border-2 border-slate-500 px-2 py-2 shadow-lg flex items-center gap-2">
                     <div className="w-10 h-10 bg-slate-800 border border-slate-600 flex items-center justify-center relative drop-shadow-md">
                         {inventory ? (
                             <div key={inventory} className="w-8 h-8 animate-pop-in"><PowerUpAsset type={inventory} /></div>
                     ...
                     <div className="flex flex-col">
                         <span className="text-xs text-gray-400">POWER-UP</span>
                         <span className="text-sm text-white">{inventory || "NONE"}</span>
                     </div>
                 </div>
            </div>
                ...
                <div className="mt-2 pointer-events-auto animate-pop-in">
                     <div className="bg-black/60 border-2 border-blue-500 px-2 py-2 shadow-lg flex items-center gap-2">
                         <div className="w-10 h-10 bg-slate-800 border border-slate-600 flex items-center justify-center relative overflow-hidden">
                             ...
                             {/* Timer Overlay for non-shield */}
                             {activeEffectRef.current.type !== 'SHIELD' && (
                                 <div 
                                    className="absolute bottom-0 left-0 h-1 bg-green-400 transition-all duration-75 ease-linear"
                                    style={{ width: `${Math.max(0, (activeEffectRef.current.timeLeft / 5000) * 100)}%` }}
                                 />
                             )}
                             {/* Shield Indicator */}
                             {activeEffectRef.current.type === 'SHIELD' && (
                                 <div className="absolute top-1 right-1 w-1.5 h-1.5 bg-blue-400 rounded-full animate-pulse shadow-[0_0_4px_#60a5fa]" />
                             )}
                         </div>
                         <div className="flex flex-col">
                             <span className="text-xs text-blue-400">ACTIVE</span>
                             <span className="text-sm text-white text-shadow-outline">{activeEffectRef.current.type.replace('_', ' ')}</span>
                         </div>
                     </div>
                </div>
            )}
      ...
      <div className={`game-container w-full h-full flex justify-center items-end pb-8 relative z-10 ${isShaking ? 'animate-shake' : ''}`}>
          ...
          >
                 ...
                 return (
                    <div key={row.id} 
                        className={`absolute left-0 w-full border-b border-black/10 overflow-hidden
                            ...
                            ${row.type === RowType.RAIL ? 'pattern-rail' : ''}
                        `}
                        style={{ 
                    ...
                    >
                        ...
                        {/* Survival Void Hint */}
                        {activeGameMode === 'SURVIVAL' && row.id < gameRef.current.autoScrollY + 1 && (
                        ...
                        {row.items && row.items.map(item => (
                            <div key={item.id}
                                ...
                                style={{
                                    left: `${item.x * GAME_CONFIG.GRID_SIZE}px`,
                                    width: `${GAME_CONFIG.GRID_SIZE}px`,
                                    height: `${GAME_CONFIG.GRID_SIZE}px`,
                                    zIndex: 18
                                }}
             ...
             >
                 {/* Active Effect Aura */}
                 {activeEffect?.type === 'SHIELD' && (
                     <div className="absolute inset-0 rounded-full border-2 border-blue-400 bg-blue-500/20 animate-pulse z-[-1]"></div>
                 )}
                 {activeEffect?.type === 'TIME_SLOW' && (
                     <div className="absolute inset-0 rounded-full border-2 border-green-400 bg-green-500/20 animate-spin-slow z-[-1]"></div>
                 )}
                 {activeEffect?.type === 'MAGNET' && (
                     <div className="absolute inset-0 rounded-full border-2 border-yellow-400 bg-yellow-500/20 animate-ping z-[-1]"></div>
                 )}
                 {activeEffect?.type === 'WATER_WALK' && (
                     <div className="absolute inset-0 -bottom-1 h-2 w-full bg-cyan-400/50 blur-sm z-[-2]"></div>
                 )}
                 {activeEffect?.type === 'DOUBLE_COINS' && (
                     <div className="absolute inset-0 rounded-full border-2 border-amber-400 bg-amber-500/10 animate-pulse z-[-1]"></div>
                 )}
      ...
      {gameState === GameState.MENU && (
        <div className="absolute inset-0 z-[70] bg-black/70 flex flex-col items-center justify-center p-4 text-center">
           ...
           <div className="w-full max-w-xs space-y-3">
              <button onClick={() => { playClick(); startGame('CLASSIC'); }} className="w-full btn-retro bg-green-500 text-white py-3 text-xl font-bold border-2 border-black hover:bg-green-400">
                CLASSIC
              </button>
              
              <button onClick={() => { playClick(); startGame('SURVIVAL'); }} className="w-full btn-retro bg-orange-600 text-white py-3 text-xl font-bold border-2 border-black hover:bg-orange-500">
                SURVIVAL
              </button>
              
               <button onClick={() => { playClick(); setGameState(GameState.CHALLENGE_MENU); }} className="w-full btn-retro bg-blue-600 text-white py-3 text-xl font-bold border-2 border-black hover:bg-blue-500">
                CHALLENGES
              </button>

              <div className="flex gap-3 mt-2">
                <button onClick={() => { playClick(); setGameState(GameState.SHOP); }} className="flex-1 btn-retro bg-purple-500 text-white py-3 text-lg font-bold border-2 border-black hover:bg-purple-400">
                  SHOP
                </button>
                <button onClick={() => { playClick(); setGameState(GameState.GAMBLE_MENU); }} className="flex-1 btn-retro bg-red-500 text-white py-3 text-lg font-bold border-2 border-black hover:bg-red-400">
                  STAKES
                </button>
              </div>
           </div>
      ...
      {/* CHALLENGE MENU */}
      {gameState === GameState.CHALLENGE_MENU && (
          <div className="absolute inset-0 z-[75] bg-[#202028] flex flex-col items-center p-4 font-pixel text-white">
              <h2 className="text-3xl text-yellow-400 font-retro mb-6 text-center">CHALLENGES</h2>
              <div className="w-full max-w-md space-y-4 overflow-y-auto pb-20">
                  {CHALLENGES.map(challenge => (
                      <div key={challenge.id} className="bg-slate-800 border-2 border-slate-600 p-4 hover:border-blue-400 transition-colors">
                          <div className="flex justify-between items-start mb-2">
                              <h3 className="text-xl font-bold text-blue-300">{challenge.title}</h3>
                              <span className={`text-xs px-2 py-1 rounded ${challenge.difficulty === 'EASY' ? 'bg-green-900 text-green-300' : challenge.difficulty === 'MEDIUM' ? 'bg-yellow-900 text-yellow-300' : 'bg-red-900 text-red-300'}`}>
                                  {challenge.difficulty}
                              </span>
                          </div>
                          <p className="text-gray-300 text-sm mb-4">{challenge.description}</p>
                          <div className="flex justify-between items-center">
                              <div className="text-yellow-400 text-sm">REWARD: ${challenge.reward}</div>
                              <button 
                                onClick={() => { playClick(); startGame('CHALLENGE', 0, challenge); }}
                                className="bg-blue-600 px-4 py-2 border-b-4 border-blue-800 active:border-b-0 active:translate-y-1 font-bold"
                              >
                                  START
                              </button>
                          </div>
              ...
              <button onClick={() => setGameState(GameState.MENU)} className="fixed bottom-4 left-1/2 -translate-x-1/2 bg-red-600 px-8 py-3 border-b-4 border-red-800 font-bold">
                  BACK
              </button>
          </div>
      )}
            ...
            <div className="bg-slate-800 border-4 border-slate-950 p-1 w-full max-w-md relative shadow-2xl scale-105">
                ...
                <div className="border-4 border-slate-900 bg-[#202028] p-6 flex flex-col items-center gap-6">
                    ...
                    {activeGameMode === 'GAMBLE' ? (
                        ...
                        <div className="text-center">
                            <h2 className="text-4xl font-retro text-yellow-400 text-shadow-outline mb-2">COMPLETED!</h2>
                            <div className="bg-black/40 p-4 border border-yellow-600 text-white">
                                <p className="mb-2">{activeChallenge?.title}</p>
                                <p className="text-green-400 text-xl">REWARD: ${activeChallenge?.reward}</p>
                            </div>
                        </div>
                    ) : (
                         // NORMAL / SURVIVAL / FAILED CHALLENGE
                         <>
                            <div className="text-center">
                                <h2 className="text-5xl font-retro text-white text-shadow-outline mb-2">GAME OVER</h2>
                                {deathReason === 'VOID' && <p className="text-red-400 font-pixel mb-2">CONSUMED BY THE VOID</p>}
                                {deathReason === 'TRAIN' && <p className="text-red-400 font-pixel mb-2">SPLATTERED BY A TRAIN</p>}
                            ...
                            {/* REWARD SECTION - Not for Challenge (handled in win) or Gamble */}
                             {activeGameMode !== 'CHALLENGE' && (
                    ...
                    <div className="flex gap-3 w-full mt-2">
                        ...
                        <button 
                            onClick={() => { 
                                playClick(); 
                                if (activeGameMode === 'GAMBLE') {
                                    setGameState(GameState.GAMBLE_MENU);
                                } else if (activeGameMode === 'CHALLENGE' && activeChallenge) {
                                    startGame('CHALLENGE', 0, activeChallenge);
                                } else {
                        ...
                        >
                            {activeGameMode === 'GAMBLE' ? 'BET AGAIN' : 'RETRY'}
                        </button>
                    </div>
                </div>
            </div>
      ...
      {gameState === GameState.GAMBLE_MENU && (
        <Gamble 
          coins={coins} 
          onStartGamble={(bet) => { playClick(); startGame('GAMBLE', bet); }}
          onClose={() => { playClick(); setGameState(GameState.MENU); }} 
        />
      )}
